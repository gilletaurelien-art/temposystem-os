...
export function DecisionsPage() {
  ...
  return <main><Section eyebrow={lang === "fr" ? "Confiance" : "Trust"} title={lang === "fr" ? "La coopération a besoin de règles visibles." : "Cooperation needs visible rules."} intro={lang === "fr" ? "La confiance repose sur des responsabilités, des limites et une mémoire que chacun peut comprendre." : "Trust rests on responsibilities, boundaries and memory everyone can understand."}><div className="civic-card-grid civic-card-grid--three">{principles.map(([title, body]) => <article key={title} className="civic-card"><h3>{title}</h3><p>{body}</p></article>)}</div><div className="trust-resources"><p className="civic-note">{lang === "fr" ? "Le registre des décisions d'architecture reste accessible dans la documentation publique du projet." : "The architecture decision register remains available in the project's public documentation."} <a href="https://github.com/gilletaurelien-art/temposystem-os/tree/main/docs/captains-log/decisions">{lang === "fr" ? "Consulter les ADR" : "Browse ADRs"} ↗</a></p><p className="civic-note">{lang === "fr" ? "La Constitution rassemble les principes, les droits et les limites qui s'imposent à TEMPOsystem." : "The Constitution brings together the principles, rights and boundaries that govern TEMPOsystem."} <a href="https://constitution.temposystem.fr">{lang === "fr" ? "Lire la Constitution" : "Read the Constitution"} ↗</a></p></div></Section></main>;
}
