  ...
  >
    ...
    <div className="civic-card-grid civic-card-grid--three pricing-grid">
      {tempoOffers.map((offer) => <article className="civic-card pricing-card" id={offer.slug} key={offer.slug}>
        ...
        <a className="civic-button civic-button--primary" href={`mailto:contact@manahome.org?subject=TEMPOSYSTEM ${offer.name}`}>{lang === "fr" ? "Étudier mon besoin" : "Discuss my needs"}</a>
      </article>)}
    </div>
